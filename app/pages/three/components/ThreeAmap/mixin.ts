...
export function useCommonMap(
    ...
    const roundData = ref(props.mapRoundData || shandongRound)
    // 内部数据
    ...
    async function getAreaData() {
        ...
        loading.value = true
        let params: any = {}
        if (props.selfMapId == -1) {
            params.id = 0 // 全国地图
        } else {
            params.areaId = props.selfMapId || region.curId
        }
        // 可选择接口调用：
        const res = (await getDicApiGeoByAreaCode(params)) as any
        if (res.code == 1) {
            mapJson = res.data
        }
        // 获取地区围栏数据
        // const roundRes = (await getDicApiGeoByAreaCode({ areaId: 0 })) as any
        // if (roundRes.code == 1) {
        //     roundData.value = roundRes.data
        // }
        loading.value = false
        return mapJson
    }
    ...
    function initThreeMap(features: any[]) {
        ...
        if (!gllayer.value) {
            gllayer.value = new AMap.GLCustomLayer({
                ...
                render: () => {
                    threeMapObject.value.render() //同步渲染3d场景
                },
            })
            mapObject.value.add(gllayer.value)
            const throttledMapZoomend = useThrottleFn(
                async (e: MouseEvent) => {},
                100
            )
