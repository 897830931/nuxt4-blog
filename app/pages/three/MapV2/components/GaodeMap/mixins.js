...
import { getDicApiGeoByAreaCode } from '@/api/common.js'
...
const BaseMixins = {
    props: {
        ...
        dataArray: {
            ...
            type: Array,
            default: () => [
                { areaId: '370800', value: 20 }, // 广州
                { areaId: '371700', value: 105 }, // 深圳
                { areaId: '371000', value: 85 }, // 佛山
                { areaId: '370400', value: 110 }, // 中山
                { areaId: '370100', value: 180 }, // 东莞
            ],
        },
        // 颜色区间
        visualList: {
            ...
            default: () => [
                ...
                { min: 100, max: 120, color: 'rgba(255, 165, 0, 0.8)' }, // 橙色：值在 100~120
                { min: 120, max: 200, color: 'rgba(255, 0, 0, 0.5)' }, // 红色：值在 120~200
            ],
        },
        // 热力图数据
        heatData: {
            ...
            for (let i = 0; i < 1000; i++) {
                heatData.data.push({
                    lng: 117.0 + Math.random()*3, // 经度在 116 到 117 之间
                    lat: 36.0 + Math.random()*4, // 纬度在 36 到 37 之间
        ...
        heatRound: {
            type: Object,
            default: () => shandongRound,
        },
        // 热力图配置 heatmap.js配置，concatiner设置无效
        heatConfig: {
            type: Object,
            default: () => ({
              radius: 20,          // 点半径，根据数据密度调整
              blur: 0.95,          // 模糊程度，0.85比较柔和
              maxOpacity: 0.8,     // 最大不透明度，保证热力图不过于刺眼
              minOpacity: 0.2,     // 最小不透明度，低值部分仍然有显示
            }) 
        },
        // 地图基础配置
        threeConfig: {
    ...
    watch: {
        ...
        visualList:{
            handler(){
              this.threeMapObject&&this.threeMapObject.updateMapStyles(this.dataArray,this.visualList)
            },
            deep:true,
        },
        dataArray:{
    ...
    methods: {
        ...
        async getAreaData() {
            ...
            if (this.selfMapId == -1) {
                ...
                // 获取其他地图
                params = {
                    id: this.selfMapId || this.region.curId,
                }
            }
        ...
        initMap(features) {
            ...
            if (allPoints.length > 0) {
                ...
                this.mapObject.setZoom(
                    this.level >= 1 ? this.zoom + 1 : this.zoom
                )
                // 移除临时多边形
            ...
            if (!this.gllayer) {
                this.gllayer = new AMap.GLCustomLayer({
                    ...
                    init: (gl) => {
                        ...
                        this.threeMapObject = markRaw(
                            new ThreeMap({
                                ...
                                config: {
                                    ...
                                    ...this.threeConfig,
                                    createTooltipElement:this.threeConfig.createTooltipElement,
                                    createLabelElement:this.threeConfig.createLabelElement,
                                },
                                renderedFun: () => {
                        ...
                        // 不知名bug，调用地图不回报错
                        this.threeMapObject.renderHeatmaps(this.heatRound,[],this.heatConfig)
                ...
                this.threeMapObject.renderHeatmaps(this.heatRound,{ data:[],max:100 },this.heatConfig);
            }
        },
        // 地图绘制后需要初始化的其他方法
        initOtherAfterArea() {},
