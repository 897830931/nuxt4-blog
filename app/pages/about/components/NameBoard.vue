...
// 注册插件
gsap.registerPlugin(DrawSVGPlugin, MotionPathPlugin, Draggable);
...
onMounted(() => {
  ...
  swatches.forEach((obj, i) => {
    ...
    obj.addEventListener("click", () => {
      gsap.to(".theWords path", { duration: 1, stroke: colorArray[i], ease: "none" });
      gsap.to(".theWords ellipse", { duration: 1, fill: colorArray[i], ease: "none" });
    });
  });
  ...
  document.querySelector("#handControl").addEventListener("click", () => toggle(iconTl));
  document.querySelector("#themeControl").addEventListener("click", () => toggle(themeTl));

  function toggle(t) { t.reversed() ? t.play() : t.reverse(); }

  Draggable.create("#speedControl", {
    type: "rotation",
    bounds: { minRotation: -136, maxRotation: 136 },
    onDrag() {
      let r = this.rotation;
      if (r < 0) speed = 1 + (r / 150);
      else speed = (r / 25) + 1;
...
/* 你原本 CSS 发我后，我会原样迁移到这里，并做 scoped/全局处理 */
