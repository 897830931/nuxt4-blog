  ...
  update(source) {
    ...
    node1Enter
      ...
      .attr("stroke", (d) => {
        ...
        return d.data.type==1?"#EEEEEE":'#4E77F0';
      })
      .attr("fill", (d) => {
        if (d.depth === 0) {
          return "#4E77F0";
        }
        return d.data.type==1?"#FFFFFF":'#F1F5FF';
      })
      .on("click", (e, d) => {
    ...
    node1Enter
      ...
      .attr("y", (d) => {
        ...
        return -4.5*this.config.rectHeight / 7;
      })
     ...
     .attr("y", (d) => {
       ...
       return -2.5*this.config.rectHeight / 7;
     })
     // 必须加 xhtml: 前缀,否则文字不显示
     .append("xhtml:p")
     .attr("width", (d) => {
     ...
     .style("box-sizing", "border-box")
     .style("color", (d) => (d.depth === 0 ? "#fff" : "#888888"))
     .style("font-size", (d) => (d.depth === 0 ? 16 : 14) + "px")
     .style("display", "flex")
     .style("align-items", "center")
     .style("justify-content", "center")
     ...
     .text(function (d) {
      if (d.depth === 0) {
       ...
       return `认缴金额:${Math.round(d.data.money*10)/10}`;
     })
     .on("click", (e, d) => {
       this.nodeClickEvent(e, d);
     });
    ...
    .attr("x", (d) => {
      ...
      return -d.data.status.length/2 * 12 - 2;
    })
    .attr("y", (d) => {
      if (d.depth === 0) {
        return -20;
      }
      return -this.config.rectHeight + 6 + 2;
    })
    // 必须加 xhtml: 前缀,否则文字不显示
    .append("xhtml:p")
    .attr("width", (d) => {
    ...
    .style("box-sizing", "border-box")
    .style("color", (d) => (d.depth === 0 ? "#fff" : "#FF4646"))
    .style("font-size", (d) => (d.depth === 0 ? 16 : 12) + "px")
    .style("display", "flex")
    .style("align-items", "center")
    .style("justify-content", "center")
    .style("text-align", "center")
    ...
    const link1Enter = link1
      ...
      .attr("stroke-width", 1)
      .attr("marker-end", `url(#${this.el}markerOfDown)`)
    ...
    const link2Enter = link2
      ...
      .attr("stroke-width", 1)
      .attr("marker-end",`url(#${this.el}markerOfUp)`);
